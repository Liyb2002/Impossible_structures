...
from . import perspective, rasterizer
...
def occlusion_raster(front_structure, back_structure):
    ...
    for i in back_structure.rect:
        point = np.array(
            [
                i.start_x + i.scale_x / 2,
                i.start_y + i.scale_y / 2,
                i.start_z + i.scale_z / 2,
                1,
            ]
        )
    ...
    for rect in front_structure.rect:
        ...
        for i in range(left_down_x, right_up_x):
            for j in range(left_down_y, right_up_y):
                if graph_back[i][j] == 1:
                    x = start_x + (i - left_down_x) * scale_x / (
                        right_up_x - left_down_x
                    )
                    y = start_y + (j - left_down_y) * scale_y / (
                        right_up_y - left_down_y
                    )
                    dist = 1000
                    for k in mid_points:
