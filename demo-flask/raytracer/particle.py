...
class Particle:
    ...
    def set_intersections(
        ...
        f_seed = gen_seed.get_seed(
            foreground_intersection, self.block_size, fore_portion, True, intersect_type
        )
        f_seed_next_possible = gen_seed.get_next_possible(
            f_seed[-1], self.block_size, True, intersect_type
        )
        f_struct = structure.Structure(
            f_seed,
            f_seed_next_possible,
            fore_portion,
            self.block_size,
            foreground_index,
        )
        ...
        b_seed_next_possible = gen_seed.get_next_possible(
            b_seed[-1], self.block_size, False, intersect_type
        )
        b_struct = structure.Structure(
            b_seed,
            b_seed_next_possible,
            back_portion,
            self.block_size,
            background_index,
        )
    ...
    def get_connecting_comp(self, connections):
        for i in connections:
            ...
            self.generate_connecting_comp(
                self.num_cc, x, y, front_z, back_z, layer1, layer2
            )
    ...
    def generate_structures(self):
        ...
        f_seed = gen_seed.get_seed(
            self.foreground_intersection, self.block_size, 1.0, True, intersect_type
        )
        f_seed_next_possible = gen_seed.get_next_possible(
            f_seed[-1], self.block_size, True, intersect_type
        )
        f_struct = structure.Structure(
            f_seed, f_seed_next_possible, 1, self.block_size, self.foreground_index
        )
        self.structures.append(f_struct)
        self.structures.append(f_struct)

        b_seed = gen_seed.get_seed(
            self.background_intersection,
            self.block_size,
            self.portion,
            False,
            intersect_type,
    ...
    def generate_connecting_comp(self, num, x, y, z_front, z_back, layer1, layer2):
        ...
        cc = connecting_comp.connecting_structure(
            x + connecting_component_x,
            y + connecting_component_y,
            z_front,
            z_back,
            self.block_size,
        )
    ...
    def is_off_screen(self):
        ...
        foreground_out_of_screen = metrics.out_of_screen(
            self.structures[1], self.foreground_max_screen, self.foreground_min_screen
        )
        background_out_of_screen = metrics.out_of_screen(
            self.structures[2], self.background_max_screen, self.background_min_screen
        )
    ...
    def parallel_score(self):
        (score, parallel_pts) = metrics.parallel_score(
            np.round(self.structures[1].history, 1),
            np.round(self.structures[2].history, 1),
        )
    ...
    def occulusion_score(self):
        ...
        for i in self.connecting_comp:
            ...
            cc_fore = np.array(
                [cc_center[0], cc_center[1], self.foreground_intersection[2]]
            )
            cc_back = np.array(
                [cc_center[0], cc_center[1], self.background_intersection[2]]
            )
        ...
        for i in self.connecting_comp:
            ...
            cc_score += metrics.occlusion_score_wDist(
                self.structures[1],
                cc_pts,
                eye,
                2,
                self.foreground_intersection[2],
                self.background_intersection[2],
            )
    ...
    def triangle_score(self):
        intersection_loc = np.array(
            [
                self.foreground_intersection[0],
                self.foreground_intersection[1],
                self.foreground_intersection[2],
                1,
            ]
        )
        ...
        for cc in self.connecting_comp:
            ...
            count = self.tirangle_occulusion(
                foreground_loc, background_loc, intersection_loc, m_view, m_proj
            )
    ...
    def tirangle_occulusion(
        self, foreground_loc, background_loc, intersection_loc, m_view, m_proj
    ):
        count = 0
        for i in self.structures[1].rect:
            ...
            if interpolation.PointInTriangle(
                point, foreground_loc, background_loc, intersection_loc
            ):
                count += 1
