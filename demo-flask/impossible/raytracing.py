...
class ortho_camera:
    def __init__(self):
        ...
        self.lookat_point = np.array(
            [0.0, 0.0, 0.0]
        )  # Look-at point (where the camera is pointing)
        ...
        ) * self.world_width  # Height of the world coordinates
    ...
    def get_ray(self, x, y):
        ...
        world_x = self.camera_position[0] + (
            ndc_x * self.world_width * 0.5 * self.right[0]
        )
        world_y = self.camera_position[1] + (
            ndc_y * self.world_height * 0.5 * self.up[1]
        )
